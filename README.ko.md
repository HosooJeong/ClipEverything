...
- 글로벌 복사/붙여넣기 단축키
- 클립보드 기록 저장 및 중복 감지
- 자동 보관 정책: 즐겨찾기가 아닌 항목은 최근 1,000개만 유지하고 오래된 항목은 자동 삭제 (즐겨찾기는 항상 보존)
- 민감 항목 제외: 비밀번호 매니저 등이 저장 제외 신호를 보낸 클립은 기록하지 않음
...
기본 단축키는 `Win+Ctrl+C`(복사 후 저장), `Win+Ctrl+V`(붙여넣기 오버레이 열기)이며 설정에서 변경할 수 있습니다. 기존 설치 사용자는 settings.json에 저장된 단축키가 그대로 유지됩니다.
